...
app.post("/api/ai/chat", async (req, res) => {
  ...
  if (!apiKey) {
    const mocks = [
      ...
      "On the financial model: Stage 1 at $400K is achievable without institutional investors — Solana Foundation grants ($50-150K), diaspora crowdfund ($100-200K), and founder capital. The working MVP is then your fundraising instrument for Stage 2 at $3M, which unlocks Multicoin Capital, Placeholder, and impact investors like Omidyar Network.",
      "The Democoin slashing mechanism uses Token-2022's permanent delegate feature. When a node's submissions consistently diverge from Anchor consensus, a governance-approved slash proposal claws back previously earned tokens. This makes deception economically costly rather than just ethically wrong.",
      "For a Solana VC pitch, lead with the engineering problem — not the political one. 'How do you create trustless consensus for 176,000 distributed data points, half offline, under adversarial conditions, in a 6-hour window?' That framing gets a technical founder leaning forward before you've mentioned Nigeria.",
      "The legal architecture has three layers: NGO registration in Nigeria (civic observer protected category), offshore foundation in Delaware holding the protocol and token, and a published Red Line Protocol that explicitly states VoteLedger observes and records — it never declares results. This makes suppression legally and reputationally expensive.",
      "Sub-Node anti-fraud relies on three mechanisms together: NIN uniqueness enforcement, two-Validator co-signature on every onboarding batch, and stake-weighted slashing tied to the sponsoring Validator's trust score. A Validator who fabricates Sub-Nodes loses standing across their entire network.",
      "On partisanship — the protocol is deliberately structurally non-partisan even though early adoption will likely skew toward already-motivated communities. By the time any party realizes VoteLedger has traction, opposing it publicly looks like opposing transparency itself.",
      "Advisor candidates are evaluated against cross-party trust, institutional credibility, and investor comfort. Oby Ezekwesili is the strongest first approach — seen as anti-corruption rather than partisan. These are strategic candidates under consideration, not confirmed advisors.",
      "I don't have live data on VoteLedger's actual current node count, fundraising status, or advisor confirmations — that's something to verify against real records. I can speak confidently to the documented protocol spec and strategy, but won't fabricate current operational numbers.",
    ];
    await new Promise(r => setTimeout(r, 600));
    return res.json({
      text: `[MOCK MODE — add GROQ_API_KEY to .env to activate the AI Advisor]\n\n${mocks[Math.floor(Math.random() * mocks.length)]}`
    });
  }
